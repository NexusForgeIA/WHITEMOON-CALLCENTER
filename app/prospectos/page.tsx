...
import { Search, ChevronDown, PhoneOutgoing, CheckCircle2 } from "lucide-react";
...
export default function ProspectosPage() {
  const [prospectos, setProspectos] = useState<CallCenterProspecto[] | null>(
    null,
  );
  ...
  const cargar = useCallback(async () => {
    ...
    else setProspectos((data ?? []) as CallCenterProspecto[]);
  }, []);
  ...
  const loading = prospectos === null && !error;
  ...
  async function lanzarLlamada() {
    ...
    try {
      ...
      setFeedback(
        `Llamada lanzada a ${target.nombre ?? target.telefono}. El estado se actualizará a "Llamando".`,
      );
      setTarget(null);
      await cargar();
    } catch (err) {
    ...
    <>
      ...
      {!loading && !error && (
        <p className="mt-3 text-xs text-muted">
          {filtered.length}{" "}
          {filtered.length === 1 ? "prospecto" : "prospectos"}
