...
import { Search, CalendarCheck, ExternalLink } from "lucide-react";
import { PageHeader } from "@/components/page-header";
import { Badge } from "@/components/badge";
import { AgenteFilter, type AgenteFiltro } from "@/components/agente-filter";
import { AGENTE_MAP } from "@/lib/agentes";
import { CALL_ESTADO, CALL_ESTADO_ORDER, PIPELINE_ESTADO } from "@/lib/labels";
import { formatDateTime, formatDuration } from "@/lib/format";
import type { CallCenterCall, CallEstado } from "@/lib/types";

export function LlamadasView({ calls }: { calls: CallCenterCall[] }) {
  ...
  return (
    <>
      ...
      <p className="mt-3 text-xs text-muted">
        {filtered.length} {filtered.length === 1 ? "llamada" : "llamadas"}
    ...
    <>
      ...
      >
        ...
        <td className="px-4 py-3">
          ...
          {c.empresa && <div className="text-xs text-muted">{c.empresa}</div>}
        </td>
        ...
        <tr className="border-b border-border/60 bg-white/[0.015]">
          <td colSpan={8} className="px-4 py-4">
            <div className="grid gap-4 md:grid-cols-2">
              {c.resumen && <Detalle titulo="Resumen">{c.resumen}</Detalle>}
